...
export class FrameJSModule {
    ...
    constructor(path, name){
        this.#name = name;
        this.#path = path;
        if (!(this.prototype instanceof FrameJSModule)){    // If this is not a subclass of FrameJSModule...
            this.#element = document.createElement('fjs-module');
            console.log("is module");
            //Classes that extend FrameJSModule will set this.#element to an instance of the corresponding
        }
    }
    get name() {
        return this.#name;
...
export class FrameJSPage extends FrameJSModule {
    ...
    load(){
        super.load().then((() => {
            this.#scripts = super.element.querySelectorAll('script');
            this.#body = super.element.querySelector('body');
        }).call(this));
    }
    newInstance(state) {
